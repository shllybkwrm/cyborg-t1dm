...
for i in range(0,len(hidden_states_test)):
    start = i*60
    stop = (i+1)*60
    if hidden_states_test[i]==0:
        color = 'g.'
        Label = 'Stable'
    elif hidden_states_test[i]==1:
        color = 'r.'
        Label = 'Rising'
    elif hidden_states_test[i]==-1:
        color = 'b.'
...
#    import pdb; pdb.set_trace()
    skinTemp.plot_date(test_timeStamps[start:stop],normskintemp[start:stop],color,label=Label)
    cgmFig.plot_date(test_timeStamps[start:stop],cgm[start:stop],color,label=Label)
pl.xlabel('Time')
fig.autofmt_xdate()
'''
pl.show()
