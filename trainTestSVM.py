...
def t1dmread(file_name):
    ...
    if file_name=='trimmedDataFiles/MYFILE106_trimmed.csv' or file_name=='trimmedDataFiles/MYFILE113_trimmed.csv' or file_name=='trimmedDataFiles/MYFILE116_trimmed.csv':
...
longTestLabels = numlabels[(int(lenTrainFull) + 1):len(numlabels)]
...
for i in range(0,len(group_test)):
    ...
    groupAssignment.extend( [(group_test[i]) for x in range(60)] )
...
#print(len(groupAssignment))
#print(len(longTestCGM))

longTestCGM = longTestCGM[:(len(groupAssignment))]
longTestLabels = longTestLabels[:(len(groupAssignment))]
#print(len(longTestLabels))

testDataOut = np.column_stack( [ longTestCGM, longTestLabels, groupAssignment ] )

#np.savetxt("resultComparison/skincgm-svm103.csv", testDataOut, '%d', delimiter=",")
